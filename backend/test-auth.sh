...
curl -s http://localhost:8080/api/system/status | head -20 || echo "Connection failed"
echo ""
echo "---"
echo ""
...
curl -s http://localhost:8080/api/team/invitations || echo "Connection failed"
echo ""
echo "---"
echo ""
...
if [ -n "$TOKEN" ]; then
  ...
  curl -s http://localhost:8080/api/team/invitations/stats \
